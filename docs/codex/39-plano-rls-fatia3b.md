...
| `hr_onboarding_plan_items_authenticated_select_by_parent_unit` | 60 | ✓ |
| `hr_workflow_template_steps_authenticated_select_by_parent_unit` | 62 | ✓ |
| `hr_evaluation_template_sections_authenticated_select_by_parent_unit` | **67** | ✗ EXCEDE |
| `hr_evaluation_template_criteria_authenticated_select_by_parent_unit` | **67** | ✗ EXCEDE |
| `hr_background_jobs_authenticated_select_by_unit` | 47 | ✓ |

**Problema:** as duas tabelas `hr_evaluation_template_{sections,criteria}` passam de 63 com
`_by_parent_unit` (Postgres truncaria silenciosamente para 63, gerando `NOTICE` e nomes
frágeis).

**Decisão (fechada):** para essas **duas** tabelas, usar o sufixo **`_by_parent`** (sem
`_unit`) → **62 chars**, preservando a semântica “derivado do pai/ancestral”. As outras duas
filhas (`hr_onboarding_plan_items`, `hr_workflow_template_steps`) mantêm `_by_parent_unit`
(cabem). Convenção resultante:

- Seção A: `<tabela>_authenticated_<comando>_by_unit`.
- Seção B (cabem): `<tabela>_authenticated_<comando>_by_parent_unit`.
- Seção B (evaluation, >63): `<tabela>_authenticated_<comando>_by_parent`.
- Seção C: `hr_background_jobs_authenticated_select_by_unit`.

> Alternativa se você preferir **uniformidade**: usar `_by_parent` nas **4** filhas. Deixo a
> decizão a seu critério; o plano abaixo adota o misto (só encurta onde é obrigatório).
...
--
-- Nomes: <tabela>_authenticated_<comando>_by_unit | _by_parent_unit | _by_parent (evaluation
-- filhas, para caber em 63 chars). Ver plano §0.3.
--
...
drop policy if exists "hr_onboarding_plan_items_authenticated_select_by_parent_unit" on public.hr_onboarding_plan_items;
create policy "hr_onboarding_plan_items_authenticated_select_by_parent_unit"
...
drop policy if exists "hr_onboarding_plan_items_authenticated_insert_by_parent_unit" on public.hr_onboarding_plan_items;
create policy "hr_onboarding_plan_items_authenticated_insert_by_parent_unit"
on public.hr_onboarding_plan_items
for insert
to authenticated
with check (
...
drop policy if exists "hr_onboarding_plan_items_authenticated_update_by_parent_unit" on public.hr_onboarding_plan_items;
create policy "hr_onboarding_plan_items_authenticated_update_by_parent_unit"
on public.hr_onboarding_plan_items
for update
to authenticated
using (
...
drop policy if exists "hr_workflow_template_steps_authenticated_select_by_parent_unit" on public.hr_workflow_template_steps;
create policy "hr_workflow_template_steps_authenticated_select_by_parent_unit"
...
drop policy if exists "hr_workflow_template_steps_authenticated_insert_by_parent_unit" on public.hr_workflow_template_steps;
create policy "hr_workflow_template_steps_authenticated_insert_by_parent_unit"
on public.hr_workflow_template_steps
for insert
to authenticated
with check (
...
drop policy if exists "hr_workflow_template_steps_authenticated_update_by_parent_unit" on public.hr_workflow_template_steps;
create policy "hr_workflow_template_steps_authenticated_update_by_parent_unit"
on public.hr_workflow_template_steps
for update
to authenticated
using (
