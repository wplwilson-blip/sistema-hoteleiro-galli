...
/**
 * DENSIDADE do alvo de toque (plano 73, D3).
 *
 * `compact` (40px) e' a barra lateral do desktop, onde o ponteiro tem precisao de pixel.
 * `comfortable` (44px) e' a gaveta, onde e' dedo -- e dedo em movimento, com o tablet numa mao
 * so'. Sao contextos de entrada diferentes; trata-los igual seria a escolha preguicosa.
 *
 * O TAMANHO VEM DE QUEM RENDERIZA -- um componente so', duas alturas. Duas copias divergiriam
 * no primeiro item que alguem acrescentasse.
 *
 * Isto NAO toca no componente `Button` (min-h-10 / sm:min-h-9): sao 341 usos em 54 arquivos, e
 * mexer neles mudaria a densidade de 34 telas que ninguem esta revisando. E' sistema de design,
 * nao casca.
 */
export type NavDensity = "compact" | "comfortable";

const ROW_HEIGHT: Record<NavDensity, string> = {
  compact: "h-10",
  comfortable: "h-11"
};
...
function SidebarItem({
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={cn(
        "flex items-center gap-3 rounded-md px-3 text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground",
        ROW_HEIGHT[density],
...
/**
 * A ARVORE DE NAVEGACAO -- fonte UNICA da barra lateral e da gaveta (plano 73, D1).
 *
 * A visibilidade por permissao (`canSee`, `visibleGroupEntries`), os grupos, o item ativo pelo
 * `pathname` e o estado de aberto/fechado vivem AQUI, e os dois consumidores renderizam a mesma
 * coisa.
 *
 * E' o desenho que nao pode desincronizar. Uma barra inferior com lista curada -- a alternativa
 * descartada na D1 -- teria uma segunda lista, que diverge no dia em que alguem acrescenta um
 * item. Divergencia de navegacao e' invisivel ate' alguem nao achar a tela.
 */
export function AppNavTree({
  density = "compact",
  onNavigate
}: {
  density?: NavDensity;
  ...
  return (
    <nav ref={navRef} className="min-h-0 flex-1 space-y-2 overflow-y-auto px-3 py-4" data-testid="app-nav-tree">
      {mainItems.filter((item) => canSee(permissions, item)).map((item) => (
        <SidebarItem key={item.label} item={item} active={isLinkActive(pathname, item)} density={density} onNavigate={onNavigate} />
      ))}
        ...
        return (
          <div key={group.label} className="space-y-1">
            <button
              ...
              // `aria-expanded` conta ao leitor de tela se o grupo esta aberto -- e e' o que o
              // E2E le' para expandir a arvore inteira sem depender da ordem dos cliques.
              aria-expanded={isOpen}
              className={cn(
                "flex w-full items-center gap-3 rounded-md px-3 text-left text-sm font-medium text-muted-foreground transition-colors hover:bg-muted hover:text-foreground",
                ROW_HEIGHT[density],
                ...
                {group.items.map((item) =>
                  isSidebarLink(item) ? (
                    <SidebarItem
                      ...
                      item={item}
                      active={isLinkActive(pathname, item)}
                      density={density}
...
/** A marca, compartilhada pela barra e pela gaveta -- mesma altura, mesmo alinhamento. */
