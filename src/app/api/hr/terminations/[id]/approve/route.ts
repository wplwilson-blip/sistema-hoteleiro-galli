...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  const { context, response } = await requireHrPermission(HR_PERMISSIONS.terminationsReview);
  ...
  try {
    ...
    const updated = await transitionEmployeeTermination({ context, termination, action: "approve" });
    ...
    return handleHrRouteError(error, "Nao foi possivel aprovar desligamento.");
  }
}
