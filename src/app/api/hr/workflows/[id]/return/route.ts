...
import {
  ...
  applyReturnStepRpc,
  buildReturnStepRpcPayload,
  createWorkflowActionRequestHash,
  getRequiredWorkflowIdempotencyKey,
  mapWorkflowRpcError,
  parseReturnStepPayload,
  resolveReturnStepIdempotencyReplay
...
const workflowRowSchema: z.ZodType<HrWorkflowRow> = z.object({
  ...
  escalation_last_at: z.string().nullable(),
  escalation_count: z.number(),
  escalation_max_level: z.number(),
...
const returnableWorkflowStatuses = new Set<string>(["in_progress", "waiting_approval"]);
...
async function loadWorkflowForReturn(context: HrRequestContext, workflowId: string) {
  ...
  if (error) {
    logHrApiError("workflows.return.workflow_lookup_failed", error);
  ...
  if (!parsed.success) {
    logHrApiError("workflows.return.workflow_parse_failed", { message: parsed.error.message });
    workflowMutationError("INTERNAL_ERROR", "Workflow invalido para devolucao.", 500);
  }

  return parsed.data;
}

async function assertActorCanReturnWorkflowUnit(context: HrRequestContext, unitId: string) {
  ...
  if (actorError) {
    logHrApiError("workflows.return.actor_lookup_failed", actorError);
  ...
  if (linkError) {
    logHrApiError("workflows.return.actor_unit_link_lookup_failed", linkError);
...
function assertWorkflowCanReturn(workflow: HrWorkflowRow) {
  if (!returnableWorkflowStatuses.has(workflow.status)) {
    workflowMutationError("WORKFLOW_STATUS_INVALID", "Workflow nao esta ativo para devolucao de etapa.", 409);
  }
}
async function assertStepCanReturn(input: {
  ...
  if (!firstActionableStep || firstActionableStep.id !== input.step.id) {
    workflowMutationError("STEP_OUT_OF_ORDER", "Devolva a etapa pendente anterior antes desta etapa.", 409);
  }
...
async function loadWorkflowDetailPayload(context: HrRequestContext, workflowId: string) {
  const workflow = await loadWorkflowForReturn(context, workflowId);
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const payload = parseReturnStepPayload(rawPayload);
    const idempotencyKey = getRequiredWorkflowIdempotencyKey(request);
    const workflow = await loadWorkflowForReturn(context, workflowId);

    assertWorkflowUnitScope(context, workflow.unit_id);
    await assertActorCanReturnWorkflowUnit(context, workflow.unit_id);

    const rpcPayload = buildReturnStepRpcPayload(payload);
    ...
    const replay = await resolveReturnStepIdempotencyReplay({
    ...
    assertWorkflowCanReturn(workflow);
    ...
    const delegation = await assertStepCanReturn({
      context,
      workflow,
      step,
      steps,
    ...
    const result = await applyReturnStepRpc({
      supabase: context.supabase,
      context,
      organizationId: workflow.organization_id,
      unitId: workflow.unit_id,
    ...
    if (!result.ok) {
      throw mapWorkflowRpcError(result, "Nao foi possivel devolver a etapa.");
    }
    ...
    return handleHrWorkflowRouteError(error, "Nao foi possivel devolver a etapa do workflow de RH.");
  }
}
