...
import { conductListSelect, loadEmployeeConduct, redactEmployeeConduct, registerConductReview, statusForConductAction, type EmployeeConductRow } from "@/lib/hr/employee-conduct";
...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (conduct.status === "cancelled") return hrApiError("Registro ja cancelado.", 422);

    const { data, error } = await context.supabase
      .from("employee_conduct_records")
      .update({ status: statusForConductAction("cancelled"), updated_by: context.session.user.id })
      .eq("id", id)
      .select(conductListSelect)
      .single();

    if (error) {
      logHrApiError("conduct.cancel_failed", error);
      return hrApiError("Nao foi possivel cancelar conduta.", 500);
    }

    await registerConductReview({ context, conduct, action: "cancelled", comments: payload.comments });
    ...
    return handleHrRouteError(error, "Nao foi possivel cancelar conduta.");
  }
}
