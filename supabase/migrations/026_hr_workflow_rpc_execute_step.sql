-- RH-6G.1 - RPC da workflow engine com suporte a create_workflow e execute_step.
-- Mantem create_workflow preservado e adiciona execucao transacional de etapa.
...
begin
  ...
  if v_action is null or v_action not in ('create_workflow', 'execute_step') then
  ...
  if v_action = 'execute_step'
    and (p_workflow_id is null or p_step_id is null) then
    return jsonb_build_object(
      'ok', false,
      'error_code', 'INVALID_PAYLOAD',
      'message', 'execute_step exige workflow_id e step_id.',
      'retryable', false
    );
  end if;
  ...
  values (
    p_organization_id,
    p_unit_id,
    case when v_action = 'execute_step' then p_workflow_id else null end,
    p_actor_user_id,
    v_action,
...
comment on function public.hr_workflow_apply_action(
  ...
  'Workflow engine transacional de RH. Nesta versao suporta create_workflow e execute_step; demais acoes retornam INVALID_ACTION.';
