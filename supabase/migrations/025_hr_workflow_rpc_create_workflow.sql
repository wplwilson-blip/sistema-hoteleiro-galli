-- RH-6E - RPC skeleton da workflow engine com suporte somente a create_workflow.
-- APIs mutaveis Node serao criadas em etapa futura.
...
begin
  if p_action is distinct from 'create_workflow' then
  ...
  if p_workflow_id is not null or p_step_id is not null then
    return jsonb_build_object(
      'ok', false,
...
comment on function public.hr_workflow_apply_action(
  ...
  'Workflow engine transacional de RH. Nesta versao suporta somente create_workflow; demais acoes retornam INVALID_ACTION.';
