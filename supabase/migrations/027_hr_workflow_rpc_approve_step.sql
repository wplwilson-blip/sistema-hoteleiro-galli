-- RH-6H - RPC da workflow engine com suporte a approve_step.
-- Mantem create_workflow e execute_step preservados e adiciona aprovacao transacional de etapa.
...
begin
  ...
  if v_action is null or v_action not in ('create_workflow', 'execute_step', 'approve_step') then
  ...
  if v_action in ('execute_step', 'approve_step')
...
comment on function public.hr_workflow_apply_action(
  ...
  'Workflow engine transacional de RH. Nesta versao suporta create_workflow, execute_step e approve_step; demais acoes retornam INVALID_ACTION.';
