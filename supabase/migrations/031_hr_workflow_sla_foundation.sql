-- RH-7A - Fundacao de SLA e deadlines para Workflow Engine RH.
-- Adiciona campos persistidos de SLA em workflows/steps e preserva a RPC transacional existente.
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'hr_workflows_sla_status_check'
      and conrelid = 'public.hr_workflows'::regclass
  ) then
    alter table public.hr_workflows
      add constraint hr_workflows_sla_status_check check (
        sla_status is null
        or sla_status in ('on_time', 'warning', 'overdue', 'completed_on_time', 'completed_late', 'cancelled')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'hr_workflows_sla_minutes_check'
      and conrelid = 'public.hr_workflows'::regclass
  ) then
    alter table public.hr_workflows
      add constraint hr_workflows_sla_minutes_check check (
        sla_minutes is null or (sla_minutes > 0 and sla_minutes <= 525600)
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'hr_workflow_steps_sla_status_check'
      and conrelid = 'public.hr_workflow_steps'::regclass
  ) then
    alter table public.hr_workflow_steps
      add constraint hr_workflow_steps_sla_status_check check (
        sla_status is null
        or sla_status in ('on_time', 'warning', 'overdue', 'completed_on_time', 'completed_late', 'cancelled')
      );
  end if;
  ...
  if not exists (
    select 1
    from pg_constraint
    where conname = 'hr_workflow_steps_sla_minutes_check'
      and conrelid = 'public.hr_workflow_steps'::regclass
  ) then
    alter table public.hr_workflow_steps
      add constraint hr_workflow_steps_sla_minutes_check check (
        sla_minutes is null or (sla_minutes > 0 and sla_minutes <= 525600)
      );
  end if;
end $$;

create index if not exists hr_workflows_sla_due_at_idx
  on public.hr_workflows (sla_due_at)
  where deleted_at is null;

create index if not exists hr_workflows_unit_sla_status_due_at_idx
  on public.hr_workflows (unit_id, sla_status, sla_due_at)
  where deleted_at is null;

create index if not exists hr_workflow_steps_sla_due_at_idx
  on public.hr_workflow_steps (sla_due_at)
  where deleted_at is null;

create index if not exists hr_workflow_steps_workflow_sla_status_due_at_idx
  on public.hr_workflow_steps (workflow_id, sla_status, sla_due_at)
  where deleted_at is null;
...
comment on function public.hr_workflow_apply_action(
  ...
  'Workflow engine transacional de RH. Nesta versao suporta create_workflow, execute_step, approve_step, reject_step, return_step e cancel_workflow; demais acoes retornam INVALID_ACTION.';
